...
// Vérification email
router.post('/verify-email', authController.verifierEmail);
router.post('/resend-code', authController.renvoyerCode);
...
// ============================================
// SETUP MOT DE PASSE AGENT (invitation)
// ============================================

// Vérifier validité du token d'invitation
router.get('/agent-setup/:token', async (req, res) => {
  try {
    const hashedToken = crypto.createHash('sha256').update(req.params.token).digest('hex');
    const agent = await User.findOne({
      invitation_token: hashedToken,
      invitation_token_expire: { $gt: Date.now() },
      invitation_acceptee: false,
      role: 'agent',
    }).select('prenom nom email etablissement_id');

    if (!agent) {
      return res.status(400).json({ success: false, message: 'Lien invalide ou expiré.' });
    }

    res.json({ success: true, data: { prenom: agent.prenom, nom: agent.nom, email: agent.email } });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});

// Créer le mot de passe (finaliser l'invitation)
router.post('/agent-setup/:token', async (req, res) => {
  try {
    const { mot_de_passe } = req.body;
    ...
    agent.mot_de_passe = mot_de_passe;
    agent.invitation_token = undefined;
    agent.invitation_token_expire = undefined;
    agent.invitation_acceptee = true;
    agent.statut = 'actif';
    await agent.save();
    ...
    res.json({
      success: true,
      message: 'Mot de passe créé ! Bienvenue sur FileZen.',
      token,
      data: agent,
    });
  } catch (error) {
